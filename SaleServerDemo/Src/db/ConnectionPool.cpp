...
public:
    /**
     * @brief   removeUseConName 删除无效连接方法
     * @param   name 连接的名称
     * @return  bool 删除是否成功
     */
    bool removeUseConName(const QString &name);
};
...
bool ConnectionPoolPrivate::removeUseConName(const QString &name)
{
    const int i = usedConnectionNames.indexOf(name);
    if (i < 0)
        return false;
    usedConnectionNames.remove(i);
    return true;
...
QSqlDatabase ConnectionPool::openConnection() {
    ...
    if (d->semaphore->tryAcquire(1, d->maxWaitTime)) {
        ...
        if (!db.isOpen()) {
            ...
            d->removeUseConName(connectionName);
            ConnectionPoolPrivate::mutex.unlock();
