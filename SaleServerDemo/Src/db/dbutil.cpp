...
QStringList DBUtil::selectMuFieldsString(const QString &sql, quint8 fieldCount,
                                         const QVariantMap &params)
{
    ...
    if (query.exec()){
        while(query.next()){
            while (fieldCount > 0){
                strList.append(query.value(i++).toString());
                fieldCount--;
            }
        }
    }
...
QList<QStringList> DBUtil::selectMuFieldsStrings(const QString &sql,
                                                 quint8 fieldCount,
                                                 const QVariantMap &params)
{
    QStringList strList;
